...
void timecor(char* filename, char* cutname, int afp, int iter){
  ...
  if(afp>0){
    if(iter == 0){
      h[0] = (TH2F*)f->Get(Form("afp_vs_obj_%s",cutname));
      h[1] = (TH2F*)f->Get(Form("afp_vs_xfp_%s",cutname));
      h[2] = (TH2F*)f->Get(Form("afp_vs_objtac_%s",cutname));
      h[3] = (TH2F*)f->Get(Form("afp_vs_xfptac_%s",cutname));
    }
    else{
      h[0] = (TH2F*)f->Get(Form("afp_vs_objC_%s",cutname));
      h[1] = (TH2F*)f->Get(Form("afp_vs_xfpC_%s",cutname));
    ...
    if(iter == 0){
      h[0] = (TH2F*)f->Get(Form("x_vs_obj_%s",cutname));
      h[1] = (TH2F*)f->Get(Form("x_vs_xfp_%s",cutname));
      h[2] = (TH2F*)f->Get(Form("x_vs_objtac_%s",cutname));
      h[3] = (TH2F*)f->Get(Form("x_vs_xfptac_%s",cutname));
    }
    else{
      h[0] = (TH2F*)f->Get(Form("x_vs_objC_%s",cutname));
      h[1] = (TH2F*)f->Get(Form("x_vs_xfpC_%s",cutname));
...
void iccor(char* filename){
  ...
  //adjust here!
  TH2F *h = (TH2F*)f->Get("ICde_vs_x0_in68NioutB");
  ...
  //adjust here!
  fu->SetParameter(2,100); //x0
  fu->SetParameter(1,0.0002); //p
  fu->SetParameter(0,790); //E
  ...
  //adjust here!
  TH2F *unc = (TH2F*)f->Get("ICde_vs_x0_in68Ni");
